...
export function ProductCard({ product }: ProductCardProps) {
  return (
    <Card className="group industrial-panel overflow-hidden rounded-[24px] border-transparent bg-white/88 transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_28px_70px_rgba(17,24,39,0.12)]">
      <div className="grid grid-cols-[120px_1fr] gap-4 p-3 md:block md:p-0">
        <div className="relative min-h-[138px] overflow-hidden rounded-[20px] md:aspect-[4/3] md:rounded-[24px]">
          <Image
            src={product.image}
            alt={product.name}
            fill
          ...
          <div className="absolute bottom-3 left-3 hidden items-center gap-2 rounded-full bg-black/58 px-3 py-2 text-xs text-white backdrop-blur-xl md:inline-flex">
            <Clock3 className="h-3.5 w-3.5" />
            Fast dispatch available
          </div>
        </div>
        <CardContent className="flex min-w-0 flex-col justify-between space-y-3 p-0 md:space-y-4 md:p-6">
